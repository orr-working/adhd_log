...
폰만 있고 맥이 없다면 → **맥 없이 TestFlight 로 폰에 설치**: [TESTFLIGHT.md](TESTFLIGHT.md) 참고.
(GitHub Actions 가 클라우드에서 빌드 → 위젯 포함 그대로 폰에 설치)

아래는 맥에서 직접 빌드하는 방법입니다.
...
│  ├─ Assets.xcassets           # 앱 아이콘 · 액센트 컬러
│  ├─ Support/                  # Haptics, NotificationManager
│  ├─ Chat/                     # 채팅 로직 (UI 아님)
...
- ✅ 검색 & 필터 / 통계·회고 화면 (구현됨)
- ✅ 매일 부드러운 리마인더 + 설정 화면 (구현됨)
- ✅ 앱 아이콘 · 햅틱 피드백 · 네이티브 네비게이션 정돈 (구현됨)
- 음성 메모 / 받아쓰기로 기록
- 책/영화 제목 자동완성(외부 API)
